...
namespace Finance.Nomenclature.API.Http;

/// <summary>
/// Outbound <see cref="DelegatingHandler"/> that forwards the inbound caller's bearer token onto the
/// Warehouse Nomenclature proxy request (SDD-NOM-001 §2.3). Service-to-service JWT is deferred until
/// <c>SDD-INT-WH-002</c> is drafted; until then the originating user's <c>Authorization</c> header is
/// copied onto the outbound call so Warehouse authorizes the request as that user.
/// </summary>
