...
/// <summary>
/// Domain event published through the transactional outbox when a chart-of-accounts account is
/// deactivated (the <c>IsActive</c> transition from <c>true</c> to <c>false</c> via update) — the only
/// retirement path, since hard delete is forbidden (SDD-ACCT-001 §2.4, §2.5, §2.8, SDD-INFRA-006 §2.2).
...
/// </summary>
public sealed record AccountDeactivatedEvent : IFinanceEvent
{
    /// <inheritdoc />
    ...
    /// <summary>Surrogate identifier of the deactivated account.</summary>
