...
> Status: **Implemented** (Phase-6 `implement-frontend` shipped 2026-06-10: `features/invoices`, `InvoicesListPage`, `InvoiceFormDialog`/`CancelInvoiceDialog`/`CreateNoteDialog`, route + nav, EN+BG i18n). Vitest 127/127 green (48 for this feature); `tsc -b` + `vite build` clean. `ui-validate` (Chrome DevTools MCP) passed the UI-owned checks (boot, nav, EN↔BG i18n with no raw keys, density, dialogs, client zod validation, error-toast mapping via `getApiErrorMessage`, per-request `X-Correlation-ID`); the one finding — the line-totals preview not recomputing — was fixed (`watch('lines')` → `useWatch`) with a strengthened regression test. **Deferred:** live golden-path CRUD / populated grid / real nomenclature options were not verifiable in-browser because no Finance backend was running during validation (the API path is covered by the SDD-INV-001/INT-WH-001 integration suite, 12/12 green); a standalone read-only detail route is out of v1 scope (§1.2).
> Owner: Frontend
