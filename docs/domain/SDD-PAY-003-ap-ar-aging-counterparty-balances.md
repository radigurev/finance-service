...
- **Invoice reversal IS excluded — and this batch ships the dependency that makes it possible.** `src/Finance.ServiceModel/Events/Invoices/` currently holds only `InvoiceConfirmedEvent`, `InvoicePostedEvent`, and `InvoiceCancelledEvent`, so before this batch a reversed invoice stayed `Posted` in `InvoiceOpenItem` forever. That is no longer accepted: the SDD-INV-001 amendment publishes a new `InvoiceReversedEvent` from its §2.7 reversal path, and SDD-PAY-002 §2.3 registers the fourth idempotent consumer that flips the mirrored `InvoiceStatus` to `Reversed` (which also removes a reversed invoice from SDD-PAY-002 §2.5's allocation-eligibility set, so real cash can no longer be matched to a document with no ledger balance). This spec adds no event and no consumer of its own; it consumes the mirrored status. Both upstream deliverables are hard prerequisites of §2.1 — if either slips, the mirrored status never becomes `Reversed` and a reversed invoice would silently keep being aged as `Posted`.
