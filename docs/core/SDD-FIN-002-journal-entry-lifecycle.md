...
**ISA-95 classification.** A `JournalEntry` is an ISA-95 **Level 4 (Business Planning & Logistics)** business-transaction record (ISA-95 / IEC 62264 Part 1, §5). The **post** and **reverse** operations are Level-4 financial business transactions that change the entry's recorded state; each MUST emit an **immutable domain event** (`JournalEntryPostedEvent`, `JournalEntryReversedEvent`) for state changes (SDD-INFRA-006) and an immutable audit row (SDD-AUDIT-001). The `JournalEntryStatusHistory` rows are **append-only Level-4 audit sub-records** of the transaction's lifecycle (who/when/correlation per transition) and are never mutated or deleted. The additive `SourceDocumentType`/`SourceDocumentId` pair (see the §2 persistence note below) is a Level-4 **source-document cross-reference**: it ties this bookkeeping transaction back to the Level-4 Document that caused it (an Invoice or a Payment), which is the ISA-95 traceability attribute linking a business transaction to its originating document. It is metadata ABOUT the transaction, not a new entity, and a reversal deliberately leaves it NULL so the offsetting entry never claims the source document's identity. No Level-3 (MES) production activity is modelled. Reference data consumed (accounts, currencies) is Level-4 master data (SDD-ACCT-001, SDD-NOM-001).
...
- **(NEW — Batch 17, owned by SDD-PAY-001 §2.5; ships with SDD-PAY-001/SDD-INV-001.)** Two additive nullable columns on `journal.JournalEntries` — `SourceDocumentType` (`NVARCHAR(40) NULL`) and `SourceDocumentId` (`UNIQUEIDENTIFIER NULL`) — plus the UNIQUE FILTERED index `IX_JournalEntries_SourceDocument` over `(SourceDocumentType, SourceDocumentId)` where both are non-null AND `[Status] = 'Posted'`, added by the migration `AddJournalEntrySourceDocument`. They are the duplicate-post backstop for the document-triggered posting handshakes: at most ONE posted entry may exist per source Document, so a redelivered `InvoiceConfirmedEvent` or `PaymentConfirmedEvent` past the Redis dedupe window cannot post a second entry (SDD-PAY-001 §2.5, SDD-INV-001 §2.5). Manual entries created through this spec's own endpoints leave both NULL, and `BuildReversal` deliberately leaves them NULL on the offsetting entry so it never claims the source document's uniqueness slot. The columns are set only at creation and are covered by the §2 immutability rule thereafter. SDD-PAY-001 §2.5 owns the definition; this bullet is the owning spec's record of a column its own table now carries.
